...
<p class="text-xs text-gray-600 mb-2 font-medium">NASA VIIRS detectó {0} anomalías.</p></div>""".format(nasa_anomalies)
...
<div class="flex flex-col"><span class="text-xs font-bold text-gray-700">Temp</span><span class="text-[10px] text-gray-500">{0}°C</span></div></div>
...
<div class="flex flex-col"><span class="text-xs font-bold text-gray-700">FWI</span><span class="text-[10px] font-bold text-red-500">{1}</span></div></div></div></div>""".format(temp, fwi_cat)
    st.markdown(html, unsafe_allow_html=True)
...
<div class="text-[10px] font-bold text-gray-600 mt-1 uppercase">Riesgo</div></div></div></div>""".format(total_incendios)
    st.markdown(html, unsafe_allow_html=True)
...
<h4 class="text-xs font-bold text-gray-800 m-0">Zona {0}</h4>
<span class="text-[9px] bg-red-500 text-white px-2 py-0.5 rounded font-black shadow-sm">{1}</span>
</div></div>""".format(ep['id'], ep['peligro'])
    html += "</div></div>"
    st.markdown(html, unsafe_allow_html=True)
...
<div class="flex justify-between items-center text-center">"""
    for i in range(5):
        ...
        html += """<div class="flex flex-col items-center"><span class="text-[10px] text-gray-500 font-bold mb-1">{0}</span>
<span class="text-sm font-black text-gray-800">{1}°</span></div>""".format(future_time.strftime("%I %p"), t_val)
    html += "</div></div>"
    st.markdown(html, unsafe_allow_html=True)
