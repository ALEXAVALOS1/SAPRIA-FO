...
def render_left_alert_card(nasa_anomalies):
    if nasa_anomalies > 0:
        ...
        html += '<h3 class="font-bold text-red-500 text-sm uppercase" style="margin:0;">Alerta Crítica</h3>'
        html += '<span class="animate-pulse h-2 w-2 rounded-full bg-red-500"></span></div>'
...
def render_factors_card(weather, fwi_cat):
    ...
    html = '<div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 mb-4">'
    html += '<h2 class="font-bold text-gray-800 flex items-center gap-2 text-xs uppercase tracking-wider mb-4 m-0">'
    html += '<span class="material-icons-outlined text-gray-700 text-sm">analytics</span> Clima en vivo</h2>'
    html += '<div class="space-y-3"><div class="flex items-center gap-3 p-2 bg-gray-50 rounded-lg">'
    html += '<span class="material-icons-outlined text-gray-400">device_thermostat</span>'
    html += f'<div class="flex flex-col"><span class="text-xs font-bold text-gray-700">Temp</span><span class="text-[10px] text-gray-500">{temp}°C</span></div></div>'
    html += '<div class="flex items-center gap-3 p-2 bg-gray-50 rounded-lg border-l-2 border-gray-700">'
    html += '<span class="material-icons-outlined text-gray-400">speed</span>'
...
def render_right_metrics(total_incendios):
    ...
    html += '<div class="grid grid-cols-2 gap-4"><div class="bg-red-50 p-3 rounded-lg border border-red-100">'
    html += f'<div class="text-3xl font-bold text-red-500">{total_incendios}</div>'
    html += '<div class="text-[10px] font-bold text-gray-600 mt-1 uppercase">Focos</div></div>'
    html += '<div class="bg-yellow-50 p-3 rounded-lg border border-yellow-100">'
    html += '<div class="text-3xl font-bold text-yellow-600">84%</div>'
