...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="SAPRIA-FO | Light Mode", page_icon="🖥️", layout="wide", initial_sidebar_state="expanded")
...
with st.sidebar:
    ...
    <div style="padding-bottom: 20px;">
        <h1 style="color:#E11D48; font-size:26px; font-weight:900; margin:0;">SAPRIA-FO</h1>
        <p style="color:#94A3B8; font-size:11px; margin:0;">Centro de Comando v9.0</p>
    </div>
    """, unsafe_allow_html=True)
    
    opcion = st.radio("Menú", ["🗺️ Mapa", "🔥 Incidentes", "⚠️ Riesgo", "🧠 Predicción", "📄 Reportes"], label_visibility="collapsed")
    if "Mapa" in opcion or "Riesgo" in opcion or "Predicción" in opcion: st.session_state['page'] = 'Monitor'
    elif "Incidentes" in opcion: st.session_state['page'] = 'Incidentes'
    elif "Reportes" in opcion: st.session_state['page'] = 'Reportes'

    st.markdown("---")
    
    # Filtros para mantener el mapa limpio (Estilo Light)
    st.markdown("<div style='font-size:10px; color:#FACC15; font-weight:bold; margin-bottom: 10px;'>FILTROS TÁCTICOS</div>", unsafe_allow_html=True)
...
if st.session_state['page'] == 'Monitor':
    col_main, col_sidebar = st.columns([2.8, 1.2]) # Ajustado para dar más espacio al panel derecho como en la imagen
...
# --- PANTALLAS SECUNDARIAS ---
    ...
    st.markdown("<h2 style='color:#0F172A;'>Generador de Informes</h2>", unsafe_allow_html=True)
        ...
        with st.spinner("Compilando..."):
            try:
                ...
                with open(pdf_path, "rb") as f:
                    st.download_button("⬇️ DESCARGAR DOCUMENTO", data=f, file_name=f"SAPRIA_Reporte_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf")
    ...
    st.markdown("<h2 style='color:#0F172A;'>Base de Datos</h2>", unsafe_allow_html=True)
    if not df.empty: st.dataframe(df[['fecha', 'colonia', 'tipo_incidente', 'causa']], use_container_width=True)

# --- FOOTER (ESTILO IMAGEN 121) ---
st.markdown("""
<div class="mega-footer">
    <div style="display:flex; justify-content:space-around; align-items:flex-start; max-width:1200px; margin:0 auto;">
        <div>
            <h2 style="color:#FACC15; margin:0; font-size:24px;">SAPRIA-FO</h2>
            <p>Sistema Municipal de Alertamiento Temprano<br>para la Prevención de Incendios.</p>
        </div>
        <div>
            <h4 style="color:#FACC15; margin-bottom:10px;">ESTRATEGIA PREVENTIVA</h4>
            <ul style="list-style:none; padding:0; line-height:1.8;">
                <li><i class="fa-solid fa-shield"></i> Protección Civil Juárez</li>
                <li><i class="fa-solid fa-truck-fire"></i> Cuerpo de Bomberos</li>
                <li><i class="fa-solid fa-clipboard-check"></i> Plan de Contingencia</li>
            </ul>
        </div>
        <div>
            <h4 style="color:#FACC15; margin-bottom:10px;">EMERGENCIAS 24/7</h4>
            <div style="font-size:48px; font-weight:900; color:white; line-height:1;">911</div>
            <p style="margin-top:5px;">Centro de Monitoreo SAPRIA-FO<br>Ciudad Juárez, Chihuahua</p>
        </div>
    </div>
    <div style="text-align:center; margin-top:40px; border-top:1px solid rgba(255,255,255,0.1); padding-top:20px;">
        © 2026 SAPRIA-FO - Municipio de Juárez - Estrategia Digital
    </div>
</div>
""", unsafe_allow_html=True)