...
try:
    ...
    # NUESTRO NUEVO MOTOR DE INTELIGENCIA ARTIFICIAL
...
# ENTRENAR IA EN VIVO: Calcular epicentros K-Means
...
with st.sidebar:
    ...
    opcion = st.radio("Menú", ["🗺️ Monitor", "🧠 Inteligencia (IA)", "📄 Reportes"], label_visibility="collapsed")
    
    st.markdown("---")
    st.markdown("<div style='font-size:10px; color:#64748B;'>CLÚSTERES ACTIVOS (K-MEANS)</div>", unsafe_allow_html=True)
    
    # Mostrar resumen de la IA en el menú
    if epicentros_ia:
        for ep in epicentros_ia[:3]: # Mostrar top 3
            ...
            <div style="background:rgba(255,255,255,0.03); padding:8px; border-radius:6px; margin-top:5px; font-size:11px; border-left: 2px solid {col};">
                <b>Zona Epicentro {ep['id']}</b><br>
                <span style="color:#94A3B8;">Concentración: {ep['weight']} eventos</span>
            </div>
            """, unsafe_allow_html=True)

# --- DASHBOARD ---
if st.session_state['page'] == 'Dashboard' or "Inteligencia" in opcion:
    render_top_navbar()
    ...
    with col_main:
        ...
        if not df_nasa.empty:
            for _, r in df_nasa.iterrows():
                ...
                folium.Marker([r['latitude'], r['longitude']], icon=folium.Icon(color="white", icon_color="red", icon="satellite-dish", prefix="fa")).add_to(m)

        # 🧠 4. CAPA DE INTELIGENCIA ARTIFICIAL (EPICENTROS) 🧠
        for ep in epicentros_ia:
            color_zona = "#E11D48" if ep['peligro'] == "CRÍTICO" else "#F59E0B"
            # Dibujar el "Radio de Acción Predictivo"
            folium.Circle(
                location=[ep['lat'], ep['lon']],
                radius=1500, # 1.5 km de radio de alerta
                color=color_zona,
                weight=1,
                fill=True,
                fill_opacity=0.1,
            ...
            # Marcador central del algoritmo
            folium.Marker(
                [ep['lat'], ep['lon']],
...
# Footer
st.markdown('<div class="mega-footer">© 2026 SAPRIA-FO • K-Means Machine Learning Engine Active 🧠</div>', unsafe_allow_html=True)